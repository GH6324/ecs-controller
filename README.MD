...
当前容器使用 Go 单进程版本：HTTP API、SQLite、任务 worker 和监控 worker 在同一个可优雅关闭的服务中运行，不需要 PHP-FPM、Nginx 或 Cron。
...
- **Frontend**: Vue 3 + 静态资源；Go 服务直接托管页面，并保留历史 `index.php?action=...` API 路径以兼容现有客户端。
